...
Le projet est construit en Node.js avec le module HTTP natif. Il ne nécessite aucune dépendance npm obligatoire et utilise 
data.json
 comme stockage persistant pour le prototype.
...
Stockage du prototype
Sauvegarde du code GitHub
Démarrage
Pour démarrer l’API, exécuter : npm start
Le serveur démarre normalement sur le port 
3000 et affiche : 
Routes disponibles
GET /health
Vérifie que l’API fonctionne correctement. Exemple : 
curl http://localhost:3000/health
GET /products
Retourne la liste de tous les produits. Exemple : 
curl http://localhost:3000/products
GET /products/:id
Consulte un seul produit à partir de son identifiant. Exemple : 
curl http://localhost:3000/products/1
Si l’identifiant n’existe pas, l’API renvoie 
Produit introuvable .
POST /products
Crée un nouveau produit.
Les champs obligatoires sont name , 
description et price . Le champ price
 doit être un nombre supérieur à 0
. Le champ currency
 est facultatif et sa valeur par défaut est XOF .
Exemple valide : 
...
.
GET /offers
Retourne la liste de toutes les offres. Exemple : 
curl http://localhost:3000/offers
GET /offers/:id
Consulte une seule offre à partir de son identifiant. Exemple : 
...
Les champs obligatoires sont name , 
description , price et productIds
. Le champ price
 doit être un nombre supérieur à 0
. Le champ productIds
 doit être un tableau contenant au moins un identifiant de produit. Le champ 
currency
 est facultatif et sa valeur par défaut est XOF .
Exemple valide : 
...
GET /orders/:id
Consulte une seule commande à partir de son identifiant. Exemple : 
curl http://localhost:3000/orders/1
POST /orders
Crée une nouvelle commande. Les champs 
customerName et amount sont obligatoires.
Exemple : 
...
Crée une session de paiement simulée pour une commande existante. Exemple : 
...
Confirme le paiement simulé d’une commande. Exemple : 
...
Génère un lien WhatsApp de livraison pour une commande existante. Exemple : 
